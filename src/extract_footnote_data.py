...
def extract_day_data(pdf_path: Path, year: int = 2026, start_page: int = 12, end_page: int = None):
    """
    Extract DAY DATA from a subset of PDF pages.
    Pages are 0-indexed: page 13 -> start_page=12
    """
    day_data = []
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_num in range(start_page, end_page):
            ...
            # Skip pages until month is detected
            ...
            for i, line in enumerate(lines):
                # Detect footnote separator lines
            ...
            # Reset trackers if footnote detected
...
def extract_day_data_split(pdf_path: Path, output_csv: Path, year: int = 2026):
    # First pass: pages 13-21
    print("🔹 Extracting pages 13-21")
    data1 = extract_day_data(pdf_path, year, start_page=12, end_page=22)

    # Second pass: pages 22-end
    print("🔹 Extracting pages 22-end")
    ...
    # Automatically add missing March 30 and 31
    ...
    print(f"✅ Merged output: {len(unique_data)} rows → {output_csv}")
...
def main():
    ...
    extract_day_data_split(Path(args.input_pdf), Path(args.output_csv), args.year)
